...
function onQuickQuestion(q) {
  handleSend({ text: q, images: null, logFile: null })
}

// 发送消息（兼容字符串和 {text, images, logFile} 结构体）
async function handleSend(payload) {
  // 快捷问题传入的可能是纯字符串
  const normalized = typeof payload === 'string' ? { text: payload, images: null, logFile: null } : payload
